...
def _category_weights():
    """
    Sum up the number of each category
    """
    category_sizes = torch.Tensor(
        [[131., 199., 177., 157., 3446., 1689., 14838.,186.],
        [379., 366., 1705., 1297., 9746., 873., 1475.,4982.], 
        [232., 257., 241., 3422., 126., 11225., 5105., 215.],
        [137., 115., 142., 3192., 3066., 10036., 3983., 152.],
        [781., 594., 1183., 4753., 664., 9038., 1394., 2416.], 
        [140., 214., 150., 4644., 11075., 1974., 182., 2444.],
...
class DogClipDataset(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        prev_imu = absolute_prev_imus.view((self.sequence_length,24))
        cur_imu = absolute_cur_imus.view((self.sequence_length,24))
        #input_with_imu = torch.cat([input,diff_imus],dim=1)
        input_with_imu = torch.cat([input,prev_imu,cur_imu],dim=1)
